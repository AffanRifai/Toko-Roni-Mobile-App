...
class _DetailTransaksiPageState extends State<DetailTransaksiPage> {
  ...
  static const Map<String, int> _deliveryRankMap = {
    'pending': 0,
    'failed': 0,
    'cancelled': 0,
    'processing': 1,
  ...
  String _savedKodePengiriman = '';
  String _savedStatusApi = 'pending';
  String _savedStatusLabel = 'Pending';
  String? _savedKurirNama;
  String? _savedKurirTelepon;

  String _normalizeStatusApi(String raw) {
  ...
  int _deliveryRankOf(String statusApi) {
    final normalized = _normalizeStatusApi(statusApi);
    return _deliveryRankMap[normalized] ?? 0;
  }

  bool get _hideShippingOptions {
    return _sudahDikirim && _deliveryRankOf(_savedStatusApi) >= 1;
  }
  ...
  void initState() {
    ...
    _alamatCtrl.text = (widget.transaksi.alamatPelanggan ?? '').trim();
    _barangDikirim = List.filled(widget.transaksi.produkList.length, false);
    _loadExistingDelivery();
  }
  ...
  Future<void> _loadExistingDelivery() async {
    final transactionId = widget.transaksi.transactionId;
    if (transactionId == null || transactionId <= 0) return;

    try {
      final existing = await DeliveryService.getDeliveryByTransactionId(
        transactionId: transactionId,
      );
      if (!mounted || existing == null) return;

      final tgl = _formatEstimatedDate(existing);
      final penerima = _extractNoteField('Penerima:', existing.catatan);
      final telepon = _extractNoteField('No Telepon:', existing.catatan);
      final kendaraan =
          existing.kendaraan ??
          _extractNoteField('Kendaraan:', existing.catatan);

      setState(() {
        _pilihanKirim = true;
        _sudahDikirim = true;

        _savedKodePengiriman = existing.kodePengiriman;
        _savedStatusApi = _normalizeStatusApi(existing.statusApi);
        _savedStatusLabel = existing.status;
        _savedNama = penerima ?? widget.transaksi.namaPelanggan;
        _savedTelepon = telepon ?? widget.transaksi.noTelepon;
        _savedAlamat = existing.tujuan;
        _savedKendaraan = kendaraan?.trim() ?? '-';
        _savedKurirNama = existing.namaKurir;
        _savedKurirTelepon = existing.nomorKurir;

        _namaPenerimaCtrl.text = _savedNama;
        _teleponPenerimaCtrl.text = _savedTelepon;
        _alamatCtrl.text = existing.tujuan;
        _tglKirimCtrl.text = tgl;
      });
    } catch (_) {
      // Tidak perlu blokir halaman jika gagal membaca data pengiriman lama.
    }
  }

  String? _extractNoteField(String key, String notes) {
  ...
  String _formatEstimatedDate(PengirimanItem delivery) {
    final raw = delivery.estimatedDeliveryRaw.trim();
    if (raw.isEmpty) return delivery.tanggalDibuat;
    final dt = DateTime.tryParse(raw)?.toLocal();
    if (dt == null) return delivery.tanggalDibuat;
  ...
  void _kirimKeLogistik() {
    ...
    if ((widget.transaksi.transactionId ?? 0) <= 0) {
      _snack(
        'ID transaksi tidak tersedia. Pengiriman harus dibuat dari data transaksi server.',
        Colors.red,
      );
      return;
    }
  ...
  Future<void> _submitPengiriman() async {
    if (_isSubmittingDelivery) return;
    final transactionId = widget.transaksi.transactionId;
    if (transactionId == null || transactionId <= 0 || _tglKirim == null) {
      return;
    }

    final selectedItems = <String>[];
    var selectedTotalQty = 0;
    for (var i = 0; i < widget.transaksi.produkList.length; i++) {
      if (_barangDikirim[i]) {
        final produk = widget.transaksi.produkList[i];
        selectedItems.add(produk.nama);
        selectedTotalQty += produk.qty;
      }
    }
    final totalQty = widget.transaksi.produkList.fold<int>(
      0,
      (sum, item) => sum + item.qty,
    );
    final totalItems = selectedTotalQty > 0 ? selectedTotalQty : totalQty;
    final biayaKirim = int.tryParse(_biayaKirimCtrl.text.trim()) ?? 0;
    ...
    try {
      final created = await DeliveryService.createDelivery(
        ...
        destination: _alamatCtrl.text.trim(),
      ...
      final kode = created.kodePengiriman.trim();
      setState(() {
        _savedNama = _namaPenerimaCtrl.text.trim().isEmpty
            ? widget.transaksi.namaPelanggan
            : _namaPenerimaCtrl.text.trim();
        _savedTelepon = _teleponPenerimaCtrl.text.trim();
        _savedAlamat = _alamatCtrl.text.trim();
        _savedKendaraan = _kendaraan ?? '-';
        _savedKodePengiriman = kode;
        _savedStatusApi = _normalizeStatusApi(created.statusApi);
        _savedStatusLabel = created.status;
        _savedKurirNama = created.namaKurir;
        _savedKurirTelepon = created.nomorKurir;
        _pilihanKirim = true;
        _sudahDikirim = true;
      });
      ...
      _snack(
        kode.isEmpty
            ? 'Pesanan berhasil dikirim ke logistik!'
            : 'Pesanan berhasil dikirim ke logistik ($kode)',
        _green,
      );
    } catch (e) {
      if (!mounted) return;
      ...
      _snack(
        msg.isEmpty ? 'Gagal mengirim pesanan ke logistik' : msg,
        Colors.red,
      );
    } finally {
      if (mounted) {
        setState(() => _isSubmittingDelivery = false);
      }
    }
  }

  String _buildDeliveryNotes({
    required List<String> selectedItems,
    required int biayaKirim,
  }) {
    final notes = <String>[];
    final catatan = _catatanCtrl.text.trim();
    if (catatan.isNotEmpty) notes.add(catatan);

    final namaPenerima = _namaPenerimaCtrl.text.trim();
    if (namaPenerima.isNotEmpty) notes.add('Penerima: $namaPenerima');

    final noTelepon = _teleponPenerimaCtrl.text.trim();
    if (noTelepon.isNotEmpty) notes.add('No Telepon: $noTelepon');

    if (_kendaraan != null && _kendaraan!.trim().isNotEmpty) {
      notes.add('Kendaraan: ${_kendaraan!.trim()}');
    }

    if (biayaKirim > 0) {
      notes.add('Biaya kirim: ${_rp(biayaKirim)}');
    }

    if (selectedItems.isNotEmpty) {
      notes.add('Barang dipilih: ${selectedItems.join(', ')}');
    }
  ...
  Widget _buildPengiriman(DetailTransaksiData t) => _Card(
    title: _hideShippingOptions ? 'Pengiriman' : 'Opsi Pengiriman',
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (!_hideShippingOptions) ...[
          // Dua opsi pilihan
          _PilihanTile(
        ...
        if (_pilihanKirim == true && !_sudahDikirim) ...[
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: _isSubmittingDelivery ? null : _kirimKeLogistik,
              icon: _isSubmittingDelivery
                  ? const SizedBox(
                      width: 16,
                      height: 16,
              ...
              label: Text(
                _isSubmittingDelivery ? 'Mengirim...' : 'Kirim ke Logistik',
                style: const TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.w600,
                ),
  ...
  Widget _buildStatusPengiriman() {
    final normalizedStatusApi = _normalizeStatusApi(_savedStatusApi);
    final rank = _deliveryRankOf(normalizedStatusApi);
    final tanggalDibuat =
        '${widget.transaksi.tanggal} ${widget.transaksi.waktu}';
    final tanggalDiproses = rank >= 1 ? tanggalDibuat : null;
    final badgeColor = switch (normalizedStatusApi) {
      'delivered' => _green,
      'failed' || 'cancelled' => const Color(0xFFE53E3E),
      'on_delivery' => _blue,
      'picked_up' => const Color(0xFF6B5CE7),
      'assigned' => const Color(0xFF805AD5),
      'processing' || 'pending' => const Color(0xFFD69E2E),
      _ => const Color(0xFF4A5568),
    };

    _ShipStatus statusOfStep(int stepRank) {
      if (rank > stepRank) return _ShipStatus.done;
      if (rank == stepRank) return _ShipStatus.active;
      return _ShipStatus.pending;
    }
    ...
    final steps = [
      _ShipStep(
        ...
        sublabel1: tanggalDibuat,
        status: statusOfStep(0),
      ),
      _ShipStep(
        icon: Icons.check_rounded,
        label: 'Diproses Logistik',
        sublabel1: tanggalDiproses,
        status: statusOfStep(1),
      ),
      _ShipStep(
        icon: Icons.person_rounded,
        label: 'Ditugaskan ke Kurir',
        sublabel1: _savedKurirNama,
        sublabel2: _savedKurirTelepon,
        status: statusOfStep(2),
      ),
      _ShipStep(
        icon: Icons.inventory_2_rounded,
        label: 'Paket Diambil Kurir',
        status: statusOfStep(3),
      ),
      _ShipStep(
        icon: Icons.local_shipping_rounded,
        label: 'Dalam Perjalanan',
        status: statusOfStep(4),
      ),
      _ShipStep(
        icon: Icons.check_rounded,
        label: 'Terkirim',
        status: statusOfStep(5),
      ),
    ];

    final overallLabel = _savedStatusLabel.isEmpty
        ? 'Pending'
        : _savedStatusLabel;
    ...
    return Column(
      ...
      children: [
        ...
        if (_savedKodePengiriman.isNotEmpty) ...[
          _row('Kode Pengiriman', _savedKodePengiriman),
          _divRow(),
        ],
        _row('Penerima', _savedNama.isEmpty ? '-' : _savedNama),
        _divRow(),
        _row('Telepon', _savedTelepon.isEmpty ? '-' : _savedTelepon),
        _divRow(),
        _row('Kendaraan', _savedKendaraan.isEmpty ? '-' : _savedKendaraan),
        _divRow(),
        _row('Alamat', _savedAlamat.isEmpty ? '-' : _savedAlamat),
      ],
    );
  }
...
class _TimelineTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Warna dot
    final dotColor = (isDone || isActive) ? _green : Colors.grey.shade300;
