...
class _RiwayatTransaksiPageState extends State<RiwayatTransaksiPage>
  ...
  Future<void> _loadTransactions() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _data = transactions.map(_mapApiToRiwayat).toList();
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
      ...
      _snack(
        msg.isEmpty ? 'Gagal memuat transaksi dari server' : msg,
        Colors.red,
      );
    } finally {
      if (mounted) {
  ...
  RiwayatTransaksiItem _mapApiToRiwayat(TransactionApiItem trx) {
    return RiwayatTransaksiItem(
      id: trx.id,
  ...
  Future<void> _hapusTransaksi(RiwayatTransaksiItem t) async {
    Navigator.pop(context);
    if (t.id == null) {
      _snack('ID transaksi tidak ditemukan', Colors.red);
      return;
    }

    try {
      await TransactionService.deleteTransaction(transactionId: t.id!);
      if (!mounted) return;
      setState(() => _data.removeWhere((item) => item.id == t.id));
      _snack('Transaksi ${t.invoice} dihapus', Colors.red);
    } catch (e) {
      if (!mounted) return;
      final msg = e.toString().replaceFirst('Exception: ', '').trim();
      _snack(msg.isEmpty ? 'Gagal menghapus transaksi' : msg, Colors.red);
    }
  }
