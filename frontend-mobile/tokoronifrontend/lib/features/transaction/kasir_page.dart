...
import 'package:tokoronifrontend/features/transaction/detail_transaksi_page.dart';
...
class _KasirPageState extends State<KasirPage>
  ...
  Future<void> _loadMembers() async {
    ...
    try {
      final members = await MemberService.getMembers(status: 'Aktif');
      if (!mounted) return;
      ...
      if (mounted) {
        _snack('Gagal memuat data member dari server', Colors.red);
      }
    } finally {
      if (mounted) {
  ...
  void _setGeneralCustomerInfo() {
    _namaCtrl.text = 'Pelanggan umum';
    _teleponCtrl.clear();
  }

  void _syncCustomerWithSelectedMember() {
    final member = _selectedMember;
    if (!_useMember || member == null) return;
    _namaCtrl.text = member.nama;
    _teleponCtrl.text = member.telepon.trim();
  }
  ...
  void _prosesTransaksi() {
    ...
    if (_metodePembayaran == 'Hutang' &&
        (!_useMember || _selectedMember == null)) {
      _snack('Transaksi hutang wajib memilih member', Colors.red);
      return;
    }
    showDialog(
      context: context,
            ...
            children: [
              ...
              Expanded(
                child: ElevatedButton(
                  ...
                  child: Text(
                    _isProcessingTransaction ? 'Memproses...' : 'Proses',
  ...
  Future<void> _submitTransaksi() async {
    ...
    if (_useMember && _selectedMember == null) {
      _snack('Silakan pilih member terlebih dahulu', Colors.red);
      return;
    }

    for (final item in _keranjang) {
      if (item.produk.id == null) {
        _snack(
          'Produk "${item.produk.nama}" belum memiliki ID dari server.',
          Colors.red,
        );
        return;
      }
    ...
    final member = _useMember ? _selectedMember : null;
    final customerName = _namaCtrl.text.trim().isNotEmpty
        ? _namaCtrl.text.trim()
        : (member?.nama ?? 'Pelanggan umum');
    final customerPhone = _teleponCtrl.text.trim().isNotEmpty
    ...
    try {
      final createdTransaction = await TransactionService.createTransaction(
        payload: CreateTransactionPayload(
          ...
          memberId: member?.id == 0 ? null : member?.id,
          paymentMethod: _mapPaymentMethodToApi(_metodePembayaran),
          discountAmount: _nilaiDiskon,
          discountPercent: _diskonPersen,
          cashReceived: _metodePembayaran == 'Tunai' ? _uangDiterima : 0,
          dueDate: _metodePembayaran == 'Hutang'
              ? _toApiDate(DateTime.now().add(const Duration(days: 30)))
              : null,
          notes: null,
          items: _keranjang
              .map(
                (item) => CreateTransactionLinePayload(
      ...
      if (!mounted) return;
      final detailData = _mapToDetailData(
        createdTransaction,
        selectedMember: member,
      );

      _snack('Transaksi berhasil diproses!', const Color(0xFF48BB78));
      _resetKeranjang();
      await _loadProducts();
      if (!mounted) return;
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => DetailTransaksiPage(transaksi: detailData),
        ),
      );
    } catch (e) {
      if (!mounted) return;
      final msg = e.toString().replaceFirst('Exception: ', '').trim();
      _snack(msg.isEmpty ? 'Gagal memproses transaksi' : msg, Colors.red);
    } finally {
      if (mounted) {
  ...
  DetailTransaksiData _mapToDetailData(
    TransactionApiItem trx, {
    MemberData? selectedMember,
  }) {
    final memberAddress = selectedMember?.alamat.trim() ?? '';
    return DetailTransaksiData(
      transactionId: trx.id,
      invoice: trx.invoiceNumber.isEmpty
          ? 'TRX-${trx.id.toString().padLeft(6, '0')}'
          : trx.invoiceNumber,
      tanggal: _toDisplayDate(trx.createdAt),
      waktu: _toDisplayTime(trx.createdAt),
      ...
      alamatPelanggan: memberAddress.isNotEmpty
          ? memberAddress
          : (trx.memberAddress.isEmpty ? null : trx.memberAddress),
    );
  }
  ...
  String _toDisplayTime(DateTime date) {
                    ...
                    children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed:
                              _keranjang.isEmpty || _isProcessingTransaction
                              ? null
                              : _showResetDialog,
              ...
              Switch(
                ...
                onChanged: (v) => setState(() {
                  ...
                  if (!v) {
                    ...
                    _syncCustomerWithSelectedMember();
                  }
                }),
                ...
                child: DropdownButton<String>(
                  ...
                  onChanged: (kode) => setState(() {
                    _selectedMember = kode == null
                        ? null
                        : _memberList.firstWhere((m) => m.kode == kode);
