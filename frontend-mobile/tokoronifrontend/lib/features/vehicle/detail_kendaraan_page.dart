...
class _DetailKendaraanPageState extends State<DetailKendaraanPage> {
  ...
  // ── Data riwayat ──────────────────────────────────────────────────────────
  List<_RiwayatDelivery> get _riwayat => _dummyRiwayatMap[_k.kode] ?? [];
  ...
  void _tandaiServis() {
    _showConfirm(
      ...
      onConfirm: () => _updateStatusAction(
        request: () => VehicleService.setMaintenance(vehicleId: _k.id),
        newStatusLabel: 'Servis',
        successMessage: '${_k.nama} ditandai sebagai Servis',
      ),
    );
  }
  ...
  void _tandaiTersedia() {
    _showConfirm(
      ...
      confirmColor: _green,
      onConfirm: () => _updateStatusAction(
        request: () => VehicleService.setAvailable(vehicleId: _k.id),
        newStatusLabel: 'Tersedia',
        successMessage: '${_k.nama} ditandai sebagai Tersedia',
      ),
    );
  }

  Future<void> _updateStatusAction({
    required Future<KendaraanItem> Function() request,
    ...
    try {
      final latest = await request();
      if (!mounted) return;

      setState(() {
        _k = _k.copyWith(
          status: latest.status.isEmpty ? newStatusLabel : latest.status,
          tanggalMaintenance: latest.tanggalMaintenance,
        );
        _isSubmittingStatus = false;
      });

      widget.onStatusChange?.call(_k.status);
      ...
      _snack(
        successMessage,
        _k.status == 'Servis' ? const Color(0xFFE53E3E) : _green,
      );
    } catch (e) {
      if (!mounted) return;
        ...
        Row(
          children: [
            if (_k.status != 'Servis') ...[
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: _isSubmittingStatus ? null : _tandaiServis,
            ...
            if (_k.status != 'Tersedia')
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: _isSubmittingStatus ? null : _tandaiTersedia,
