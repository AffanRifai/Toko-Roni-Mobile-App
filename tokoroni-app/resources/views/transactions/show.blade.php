...
@section('content')
                    ...
                    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div class="p-5">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4 space-y-2">
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Nama:</span>
                                            <span class="font-medium text-gray-900">{{ $transaction->recipient_name ?? $transaction->customer_name }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Telepon:</span>
                                            <span class="font-medium text-gray-900">{{ $transaction->recipient_phone ?? $transaction->customer_phone ?? '-' }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Alamat:</span>
                                            <span class="font-medium text-gray-900 text-right">{{ $transaction->delivery_address }}</span>
                                        </div>
                                        @if($transaction->desired_delivery_date)
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Tanggal:</span>
                                            <span class="font-medium text-gray-900">{{ $transaction->desired_delivery_date->format('d/m/Y') }}</span>
                                        </div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        <div class="flex justify-between items-center mb-3 pb-2 border-b border-gray-200">
                                            ...
                                            <span class="font-semibold text-blue-600">{{ $transaction->total_items_to_deliver }} item</span>
                                        </div>
                                        @if($transaction->items_to_deliver_list->isNotEmpty())
                                        ...
                                        <div class="flex justify-between items-center my-3 py-2 border-t border-gray-200">
                                            ...
                                            <span class="font-semibold text-green-600">{{ $transaction->total_items_taken }} item</span>
                                        </div>
                                        @if($transaction->items_taken_list->isNotEmpty())
                                            <div class="space-y-1">
                                ...
                                <!-- Biaya & Catatan -->
                                @if($transaction->delivery_fee > 0 || $transaction->delivery_notes)
                    ...
                    @if(auth()->user()->role === 'kasir' || auth()->user()->role === 'owner')
                        ...
                        @elseif(!$transaction->need_delivery && !$transaction->delivery)
                            ...
                            <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-blue-500">
                                ...
                                <form action="{{ route('delivery.request', $transaction) }}" method="POST" class="space-y-4">
                                    @csrf
                        ...
                        @elseif($transaction->delivery)
                            ...
                            <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-green-500">
                                <h3 class="font-bold text-gray-900 mb-4 flex items-center justify-between">
                                    ...
                                    <span class="px-2 py-1 rounded-full text-xs font-medium {{ $transaction->delivery->getStatusBadgeClass() }}">
                                        <i class="fas {{ $transaction->delivery->getStatusIcon() }} mr-1"></i>
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="flex items-start mb-6 relative">
                                            ...
                                            <div class="ml-4 flex-1">
                                                ...
                                                <p class="text-xs text-gray-500">{{ $transaction->delivery->created_at->format('d/m/Y H:i') }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <!-- Ditugaskan ke Kurir -->
                                        <div class="flex items-start mb-6 relative">
                                            <div class="w-8 h-8 rounded-full {{ $transaction->delivery->driver_id ? 'bg-green-500' : 'bg-gray-300' }} flex items-center justify-center text-white z-10">
                                                <i class="fas {{ $transaction->delivery->driver_id ? 'fa-check' : 'fa-user' }} text-sm"></i>
                                            </div>
                                            <div class="ml-4 flex-1">
                                                <h4 class="font-semibold text-gray-900">Ditugaskan ke Kurir</h4>
                                                @if($transaction->delivery->driver)
                                                    <p class="text-sm text-gray-800">{{ $transaction->delivery->driver->name }}</p>
                                                    <p class="text-xs text-gray-500">{{ $transaction->delivery->driver->phone ?? '-' }}</p>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex items-start mb-6 relative">
                                            ...
                                            <div class="ml-4 flex-1">
                                                ...
                                                @if($transaction->delivery->start_delivery_time)
                                                    <p class="text-xs text-gray-500">{{ $transaction->delivery->start_delivery_time->format('d/m/Y H:i') }}</p>
                                                @endif
                                            </div>
                                        </div>
                                        @endif
                                        ...
                                        <div class="flex items-start relative">
                                            ...
                                            <div class="ml-4 flex-1">
                                                ...
                                                @if($transaction->delivery->delivered_at)
                                                    <p class="text-xs text-gray-500">{{ $transaction->delivery->delivered_at->format('d/m/Y H:i') }}</p>
                                                @endif
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5">
                        ...
                        <div class="bg-gradient-to-br from-white to-gray-50 rounded-lg border border-gray-200 p-4">
                            ...
                            <div class="flex justify-between items-center py-2">
                                ...
                                <span class="font-medium text-gray-900">Kasir {{ $transaction->user->name }}</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <form id="createDeliveryForm" action="{{ route('delivery.request', $transaction) }}" method="POST" class="hidden">
        ...
        <input type="hidden" name="recipient_name" value="{{ $transaction->recipient_name }}">
        <input type="hidden" name="recipient_phone" value="{{ $transaction->recipient_phone }}">
        ...
        <input type="hidden" name="delivery_fee" value="{{ $transaction->delivery_fee }}">
        @if($transaction->items_to_deliver)
            @foreach($transaction->items_to_deliver_list as $item)
