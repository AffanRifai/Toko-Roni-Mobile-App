...
/*
|--------------------------------------------------------------------------
| PUBLIC API ROUTES (FACE RECOGNITION)
|--------------------------------------------------------------------------
*/
Route::prefix('v1')->group(function () {
    // Public face recognition endpoints (no auth required)
    Route::post('/face/compare', [AuthenticatedSessionController::class, 'compareFace'])->name('api.face.compare');
    Route::get('/faces/registered', [AuthenticatedSessionController::class, 'getRegisteredFaces'])->name('api.faces.registered');
    Route::get('/face/status', [AuthenticatedSessionController::class, 'faceStatus'])->name('api.face.status');
});
    ...
    */
    Route::middleware('role:owner,admin')->group(function () {
        ...
        Route::prefix('users')->name('users.')->group(function () {
            Route::get('/face-registration', [UserController::class, 'getUsersForFaceRegistration'])->name('face-registration');
            Route::get('/faces/descriptors', [UserController::class, 'getFaceDescriptors'])->name('faces.descriptors');
            Route::post('/faces/register', [UserController::class, 'registerFace'])->name('faces.register');
            Route::delete('/faces/{id}', [UserController::class, 'deleteFaceRegistration'])->name('faces.delete');
        });
