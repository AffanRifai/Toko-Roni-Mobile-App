                        ...
                        @if (isset($categories) && count($categories) > 0)
                            @foreach ($categories as $category)
                                <button
                                    ...
                                    <i class="fas fa-tag text-xs md:text-sm"></i> {{ $category->name }}
                                </button>
                            @endforeach
                        @endif
                    </div>
                    ...
                    <div
                        ...
                        @forelse($products as $product)
                            <div class="product-card bg-white border border-gray-200 rounded-lg md:rounded-xl p-2 md:p-3 hover:border-blue-400 hover:shadow-md transition-all duration-200 relative flex flex-col h-full min-h-[180px] md:min-h-[200px]"
                                ...
                                data-category="{{ $product->category_id ?? '' }}" data-code="{{ $product->code ?? '' }}">
                                ...
                                <div class="absolute top-1.5 md:top-2 right-1.5 md:right-2 z-10">
                                    @if ($product->stock > 20)
                                        <span
                                            ...
                                            <span class="hidden sm:inline">{{ $product->stock }}</span>
                                            <span class="sm:hidden">{{ $product->stock }}</span>
                                        </span>
                                                ...
                                                @if (isset($members) && count($members) > 0)
                                                    @foreach ($members as $member)
                                                        <option value="{{ $member->id }}"
                                                            data-limit="{{ $member->limit_kredit }}"
                                                            data-piutang="{{ $member->total_piutang }}"
                                                            data-nama="{{ $member->nama }}"
                                                            data-kode="{{ $member->kode_member }}">
                            ...
                            <div class="p-3 md:p-4 grid grid-cols-1 gap-3 md:gap-4">
                                <div>
                                    ...
                                    <input type="text" id="customer_name" name="customer_name" required
                                        ...
                                        value="{{ old('customer_name', 'Pelanggan Umum') }}">
                                </div>
                                <div>
