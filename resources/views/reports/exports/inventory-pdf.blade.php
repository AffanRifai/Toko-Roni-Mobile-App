        ...
        <tbody>
            ...
            @forelse($products ?? [] as $index => $product)
                ...
                <tr>
                    ...
                    <td class="text-center">{{ number_format($product->min_stock ?? 10) }}</td>
                    <td class="text-center">{!! $statusStok !!}</td>
                    <td class="text-right">Rp {{ number_format($product->price ?? 0, 0, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($nilaiProduk, 0, ',', '.') }}</td>
    ...
    <div style="margin-top: 30px;">
        ...
        <table>
            ...
            <tbody>
                ...
                @forelse($transactionItems ?? [] as $index => $item)
                    @php
                        $totalQty += $item->qty ?? 0;
                    ...
                    <tr>
                        <td class="text-center">{{ $index + 1 }}</td>
                        <td class="text-center">{{ $item->transaction->invoice_number ?? '-' }}</td>
                        ...
                        <td>
                            @if($item->transaction->member ?? null)
                                {{ $item->transaction->member->name }}
                            @else
                                {{ $item->transaction->customer_name ?? 'Umum' }}
                            @endif
                        </td>
                    </tr>
                @empty
    ...
    <div class="summary clearfix" style="margin-top: 60px;">
        <table>
            <tr>
                ...
                <td width="40%" class="text-right">{{ number_format($totalProducts) }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="text-right">{{ number_format($lowStockCount) }}</td>
            </tr>
            <tr>
                <th>PRODUK HABIS</th>
                <td class="text-right">{{ number_format($outOfStockCount) }}</td>
            </tr>
            <tr>
                <th>PRODUK AKTIF</th>
                <td class="text-right">{{ number_format($products->where('is_active', true)->count() ?? 0) }}</td>
            </tr>
            <tr>
    ...
    <div style="margin-top: 20px; width: 100%;">
        <table style="width: 400px; float: right; border: 2px solid #2c3e50;">
            ...
            <tr>
                ...
                <td width="40%" class="text-right">{{ number_format($totalQty) }}</td>
            </tr>
            <tr>
                <th>TOTAL TRANSAKSI ITEM</th>
                <td class="text-right">{{ number_format($transactionItems->count() ?? 0) }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="text-right">Rp {{ number_format($totalQty > 0 ? $totalSubtotal / $totalQty : 0, 0, ',', '.') }}</td>
            </tr>
        </table>
    </div>
