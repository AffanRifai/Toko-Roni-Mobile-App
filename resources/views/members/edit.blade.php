...
@section('page-subtitle', 'Perbarui informasi member')
        ...
        <form action="{{ route('members.update', $member) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-blue-50/50 rounded-xl p-4 mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-900">{{ $member->nama }}</h3>
                        <p class="text-sm text-gray-600">Kode Member: <span class="font-mono">{{ $member->kode_member }}</span></p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="tipe_member" required
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500">
                        <option value="biasa" {{ old('tipe_member', $member->tipe_member) == 'biasa' ? 'selected' : '' }}>Biasa</option>
                        <option value="gold" {{ old('tipe_member', $member->tipe_member) == 'gold' ? 'selected' : '' }}>Gold</option>
                        <option value="platinum" {{ old('tipe_member', $member->tipe_member) == 'platinum' ? 'selected' : '' }}>Platinum</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <input type="number" name="limit_kredit" value="{{ old('limit_kredit', $member->limit_kredit) }}" min="0" required
                           class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500">
                </div>
            </div>

            {{-- Informasi Piutang Saat Ini --}}
            @if($member->total_piutang > 0)
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-yellow-700">
                            Member ini memiliki piutang sebesar <strong>Rp {{ number_format($member->total_piutang) }}</strong>.
