...
@section('page-subtitle', 'Ringkasan dan statistik bisnis Anda')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-purple-50/50 to-purple-100/30 p-4 md:p-6">
    <!-- Welcome Header -->
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 md:gap-6">
            <div class="flex-1">
                <div class="flex items-center gap-3 md:gap-4">
                    <div class="relative">
                        <div class="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center shadow-lg">
                            <i class="fas fa-chart-line text-xl md:text-2xl text-white"></i>
                        </div>
                        <div class="absolute -inset-1 md:-inset-2 bg-gradient-to-r from-purple-500 to-purple-600 rounded-2xl blur-xl opacity-20"></div>
                    </div>
                    <div>
                        <h1 class="text-xl md:text-3xl lg:text-4xl font-bold text-gray-800">Selamat datang, <span class="text-purple-600">{{ Auth::user()->name }}!</span> 👋</h1>
            ...
            <div class="flex flex-col sm:flex-row gap-3 md:gap-4">
                <div class="flex items-center gap-2 md:gap-3 px-4 md:px-6 py-3 md:py-4 bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-white/50">
                    <div class="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-gradient-to-br from-purple-100 to-purple-200 flex items-center justify-center">
                        <i class="fas fa-calendar-alt text-purple-600 text-sm md:text-lg"></i>
                    </div>
                    <div>
                        <p class="text-xs md:text-sm text-gray-500 font-medium">Tanggal</p>
                        <p class="text-sm md:text-base font-semibold text-gray-800">{{ now()->translatedFormat('l, d F Y') }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-2 md:gap-3 px-4 md:px-6 py-3 md:py-4 bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-white/50">
                    ...
                    <div>
                        ...
                        <p class="text-sm md:text-base font-semibold text-gray-800" id="currentTime">{{ now()->format('H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                        ...
                        <h3 class="text-xl md:text-3xl font-bold text-gray-800 mt-1">{{ $totalUsers ?? 0 }}</h3>
                    </div>
                </div>
                <a href="{{ route('users.index') }}" class="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-blue-500 hover:text-white transition-all">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                        ...
                        <h3 class="text-xl md:text-3xl font-bold text-gray-800 mt-1">{{ $totalProducts ?? 0 }}</h3>
                    </div>
                </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl p-5 md:p-6 shadow-sm border border-white/50 hover:shadow-md transition-all">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                        ...
                        <h3 class="text-xl md:text-3xl font-bold text-gray-800 mt-1">{{ $lowStockCount ?? 0 }}</h3>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
                <div class="flex items-center text-xs md:text-sm">
                    ...
                    <span class="text-red-600 font-semibold">{{ $criticalStockCount ?? 0 }} kritis</span>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl p-5 md:p-6 shadow-sm border border-white/50 hover:shadow-md transition-all">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                        ...
                        <h3 class="text-xl md:text-3xl font-bold text-gray-800 mt-1">{{ $expiringSoonCount ?? 0 }}</h3>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
                <div class="flex items-center text-xs md:text-sm">
                    ...
                    <span class="text-amber-600 font-semibold">{{ $expiredCount ?? 0 }} expired</span>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl overflow-hidden shadow-sm border border-white/50" id="low-stock">
            <div class="p-4 md:p-6 border-b border-gray-100">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('products.index') }}" class="text-purple-600 hover:text-purple-700 font-medium text-xs md:text-sm flex items-center gap-1">
                        ...
                        <div class="flex items-start gap-3 md:gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2">
                                    <div>
                                        <h4 class="text-sm md:text-base font-semibold text-gray-900 truncate">{{ $product->name }}</h4>
                                        <p class="text-xs md:text-sm text-gray-500 mt-1">{{ $product->category->name ?? '-' }}</p>
                                    </div>
                                    <div class="text-right">
                                        <span class="inline-block px-2 py-1 rounded-lg text-xs font-medium {{ $product->stock <= 5 ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800' }}">
                                            Stok: {{ $product->stock }}
                                        </span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-4 mt-3">
                                    <div class="flex-1">
                                        <div class="w-full bg-gray-200 rounded-full h-2.5">
                                            @php $percentage = min(($product->stock / 10) * 100, 100); @endphp
                                            <div class="h-2.5 rounded-full {{ $product->stock <= 5 ? 'bg-red-500' : 'bg-amber-500' }}" style="width: {{ $percentage }}%"></div>
                                        </div>
                                    </div>
                        ...
                        <div class="flex items-start gap-3 md:gap-4">
                            <div class="relative flex-shrink-0">
                                <div class="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br
                                    @if($expiryStatus == 'expired') from-red-100 to-rose-100
                                    @elseif($expiryStatus == 'critical') from-orange-100 to-amber-100
                                    @elseif($expiryStatus == 'warning') from-yellow-100 to-amber-50
                                    @else from-blue-100 to-cyan-100 @endif
                                    flex items-center justify-center">
                                    <i class="fas fa-calendar-alt text-lg md:text-xl
                                        @if($expiryStatus == 'expired') text-red-600
                                        @elseif($expiryStatus == 'critical') text-orange-600
                                        @elseif($expiryStatus == 'warning') text-amber-600
                                    ...
                                    <div class="text-right">
                                        ...
                                        @elseif($expiryStatus == 'warning')
                                            <span class="inline-block px-2 py-1 rounded-lg text-xs font-medium bg-amber-100 text-amber-800">
                                                <i class="fas fa-calendar mr-1"></i>{{ $daysLeft }} hari
                                            </span>
                                        @else
                                            <span class="inline-block px-2 py-1 rounded-lg text-xs font-medium bg-blue-100 text-blue-800">
                                                {{ $product->expiry_date ? \Carbon\Carbon::parse($product->expiry_date)->format('d/m/Y') : 'Tidak Ada' }}
                                            </span>
                                        @endif
                                    </div>
                                </div>
                                <div class="flex flex-wrap items-center gap-4 mt-3">
                                    ...
                                    @if($product->expiry_date)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-xs text-gray-600">{{ \Carbon\Carbon::parse($product->expiry_date)->format('d M Y') }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl p-4 md:p-6 shadow-sm border border-white/50">
            ...
            <div class="mt-4 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs md:text-sm font-semibold">{{ $normalStockCount ?? 0 }} produk</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs md:text-sm font-semibold">{{ $lowStockCount ?? 0 }} produk</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs md:text-sm font-semibold">{{ $criticalStockCount ?? 0 }} produk</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl overflow-hidden shadow-sm border border-white/50">
                <div class="p-4 md:p-6 border-b border-gray-100">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between">
                        ...
                        <a href="{{ route('transactions.index') }}" class="mt-4 sm:mt-0 px-4 md:px-5 py-2 md:py-2.5 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-xl text-xs md:text-sm font-medium hover:shadow-lg transition-all inline-flex items-center gap-2">
                    ...
                    @forelse($recentTransactions ?? [] as $transaction)
                        <div class="p-4 md:p-6 hover:bg-gray-50 transition-colors group">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <div class="flex items-center gap-3 md:gap-4">
                                    <div class="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-purple-100 flex items-center justify-center text-purple-600 flex-shrink-0">
                                        <i class="fas fa-receipt text-lg md:text-xl"></i>
                                    </div>
                                    <div>
                                        <h4 class="font-semibold text-gray-900 text-sm md:text-base">Transaksi #{{ $transaction->invoice_display ?? $transaction->id }}</h4>
                                        <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mt-1">
                                            <span class="text-xs md:text-sm text-gray-500 flex items-center gap-1">
                                                <i class="fas fa-user"></i> {{ $transaction->customer_name ?? 'Pelanggan Umum' }}
                                            </span>
                                            <span class="text-xs md:text-sm text-gray-500 flex items-center gap-1">
                                ...
                                <div class="flex items-center justify-between sm:justify-end gap-4">
                                    <div class="text-right">
                                        <div class="text-lg md:text-2xl font-bold text-gray-900">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</div>
                                    </div>
                                    <a href="{{ route('transactions.show', $transaction) }}" class="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 hover:bg-purple-500 hover:text-white transition-all">
        ...
        <div>
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl p-4 md:p-6 shadow-sm border border-white/50 h-full">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                    <a href="{{ route('transactions.create') }}" class="group p-4 rounded-2xl bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-100 hover:shadow-md transition-all">
                    ...
                    <a href="{{ route('products.create') }}" class="group p-4 rounded-2xl bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100 hover:shadow-md transition-all">
                    ...
                    <a href="{{ route('products.index') }}" class="group p-4 rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100 hover:shadow-md transition-all">
                    ...
                    <a href="{{ route('reports.sales') }}" class="group p-4 rounded-2xl bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-100 hover:shadow-md transition-all">
                ...
                <div class="mt-6 pt-4 border-t border-gray-100">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-gray-800">{{ $totalStock ?? 0 }} unit</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-xs md:text-sm text-gray-600">Nilai Stok</span>
                            <span class="font-semibold text-gray-800">Rp {{ number_format($totalStockValue ?? 0, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-xs md:text-sm text-gray-600">Produk per Kategori</span>
                            <span class="font-semibold text-emerald-600">{{ $productCategories ?? 0 }} kategori</span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const initialSales = {!! json_encode($chartSales) !!};
        const initialStockOut = {!! json_encode($chartStockOut) !!};

        const normalStock = {{ $normalStock }};
