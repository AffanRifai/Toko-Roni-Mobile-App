...
@section('page-subtitle', 'Manajemen Inventori & Stok')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div class="space-y-4 md:space-y-6">
            ...
            <div class="glass-effect rounded-3xl p-4 md:p-6 shadow-elegant">
                ...
                <div class="space-y-3">
                    @forelse($recentStockUpdates ?? [] as $update)
                    <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-lg {{ $update->type === 'in' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600' }} flex items-center justify-center">
                                <i class="fas fa-{{ $update->type === 'in' ? 'arrow-down' : 'arrow-up' }} text-xs"></i>
                            </div>
                            <div>
                        ...
                        <div class="text-right">
                            <span class="text-sm font-bold {{ $update->type === 'in' ? 'text-emerald-600' : 'text-blue-600' }}">
                                {{ $update->type === 'in' ? '+' : '-' }}{{ $update->quantity }}
                            </span>
                            <p class="text-xs text-gray-500">Stok: {{ $update->new_stock }}</p>
                        </div>
                    </div>
                    @empty
    ...
    <div class="mt-6 md:mt-8">
        <div class="glass-effect rounded-3xl p-4 md:p-6 shadow-elegant">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('categories.index') }}" class="text-amber-600 hover:text-amber-700 text-sm font-medium">
                    Kelola Kategori
                </a>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                ...
                <div class="group cursor-pointer">
                    <div class="aspect-square rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 flex flex-col items-center justify-center p-4 mb-2 group-hover:shadow-md transition-shadow">
                        ...
                        <span class="text-sm font-bold text-gray-800">{{ $category->products_count ?? 0 }}</span>
                    </div>
                    <div class="text-center">
                        <p class="text-xs font-medium text-gray-900 truncate">{{ $category->name }}</p>
                        <p class="text-xs text-gray-500 mt-1">{{ $category->low_stock_count ?? 0 }} low stock</p>
                    </div>
                </div>
                @empty
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const lowStockItems = {{ $lowStockProducts ?? 0 }};
        if (lowStockItems > 0) {
