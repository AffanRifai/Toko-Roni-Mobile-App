                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-gray-600">
                            Registrasi biometrik untuk <span class="font-semibold">{{ $user->name }}</span>
                        </p>
                    </div>
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    @if ($user->face_registered_at)
                        <div class="text-right">
                            ...
                            <p class="font-semibold text-gray-900">{{ $user->face_registered_at->format('d/m/Y H:i') }}</p>
                            @if ($user->face_score)
                                <div class="flex items-center justify-end mt-1">
                                    <span class="text-sm font-medium text-green-600 mr-2">
                                        Skor: {{ number_format($user->face_score * 100, 1) }}%
                                    </span>
                                    <div class="w-24 bg-gray-200 rounded-full h-2">
                                        <div class="bg-green-500 h-2 rounded-full"
                                            style="width: {{ $user->face_score * 100 }}%"></div>
                                    </div>
                                </div>
                            @endif
                        </div>
    ...
    <script>
        ...
        const FaceRegistration = {
            ...
            async captureFace() {
                ...
                const confirmed = await this.showConfirm(
                    'Konfirmasi Simpan',
                    `Simpan wajah dengan skor ${Math.round(this.currentScore * 100)}% untuk {{ $user->name }}?`
                );
                ...
                try {
                    ...
                    const response = await fetch("{{ route('users.face.store', $user->id) }}", {
        ...
        async function resetFaceRegistration() {
            ...
            const confirmed = await FaceRegistration.showConfirm(
                ...
                `Data wajah untuk {{ $user->name }} akan dihapus permanen. Lanjutkan?`
            );

            if (!confirmed) return;

            try {
                const response = await fetch("{{ route('users.face.destroy', $user->id) }}", {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': "{{ csrf_token() }}",
                        'Accept': 'application/json'
