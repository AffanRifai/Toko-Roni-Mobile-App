...
@section('page-title', 'Tugas Pengiriman Saya')
@section('page-subtitle', 'Daftar pengiriman yang ditugaskan kepada Anda')
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                ...
                <h3 class="text-3xl font-bold text-gray-900 mb-1">{{ number_format($stats['assigned'] ?? 0) }}</h3>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                ...
                <h3 class="text-3xl font-bold text-gray-900 mb-1">{{ number_format($stats['completed'] ?? 0) }}</h3>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($deliveries as $delivery)
                        <tr class="hover:bg-blue-50/30 transition-colors">
                            <td class="px-6 py-4 font-medium text-gray-900">{{ $delivery->delivery_code }}</td>
                            <td class="px-6 py-4 text-gray-700">{{ Str::limit($delivery->destination, 40) }}</td>
                            <td class="px-6 py-4">
                                @php
                                    $statusColors = [
                            ...
                            <td class="px-6 py-4 space-x-2">
                                <a href="{{ route('delivery.show', $delivery->id) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 text-sm font-medium">
                                    <i class="fas fa-eye mr-1"></i> Detail
                                </a>
                                
                                @if($delivery->status === 'assigned')
                                    <form action="{{ route('delivery.accept', $delivery->id) }}" method="POST" class="inline-block">
                                        @csrf
                                ...
                                @if(in_array($delivery->status, ['picked_up', 'on_delivery']))
                                    <form action="{{ route('delivery.update-status', $delivery->id) }}" method="POST" class="inline-flex flex-col gap-2">
                                        @csrf
