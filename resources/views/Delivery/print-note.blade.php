...
<head>
    ...
    <title>Surat Jalan - {{ $delivery->delivery_code }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="print-container">
        ...
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $delivery->delivery_code }}</span>
            </div>
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $delivery->origin }}</span>
            </div>
            ...
            @if ($delivery->estimated_delivery_time)
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $delivery->estimated_delivery_time->format('d/m/Y H:i') }}</span>
                </div>
            @endif
        </div>
        ...
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $delivery->transaction->invoice_number }}</span>
            </div>

            <div class="info-row">
                <span class="info-label">Customer</span>
                <span class="info-value">{{ $delivery->transaction->customer_name }}</span>
            </div>

            @if ($delivery->transaction->customer_phone)
                <div class="info-row">
                    <span class="info-label">No. Telepon</span>
                    <span class="info-value">{{ $delivery->transaction->customer_phone }}</span>
                </div>
            @endif
            ...
            <div class="info-row">
                ...
                <span class="info-value">Rp
                    {{ number_format($delivery->transaction->total_amount, 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                ...
                @if ($delivery->total_items > 0)
                    <tr style="font-weight: bold; background: #f2f2f2;">
                        <td colspan="2" style="text-align: right;">Total Item:</td>
                        <td style="text-align: center;">{{ $delivery->total_items }}</td>
                        <td></td>
                    </tr>
                @endif
            </tbody>
        ...
        <!-- Informasi Tambahan -->
        @if ($delivery->total_weight > 0 || $delivery->total_volume > 0)
            <div style="margin: 10px 0; padding: 10px; background: #f9f9f9;">
                @if ($delivery->total_weight > 0)
                    <div style="display: inline-block; margin-right: 20px;">
                        <strong>Total Berat:</strong> {{ number_format($delivery->total_weight, 1) }} kg
                    </div>
                @endif
                @if ($delivery->total_volume > 0)
                    <div style="display: inline-block;">
                        <strong>Total Volume:</strong> {{ number_format($delivery->total_volume, 1) }} m³
                    </div>
                @endif
            </div>
        @endif
