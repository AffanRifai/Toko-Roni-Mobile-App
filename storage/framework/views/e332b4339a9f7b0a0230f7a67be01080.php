<?php $__env->startSection('title', 'Detail Pengiriman #' . $delivery->delivery_code); ?>
<?php $__env->startSection('page-title', 'Detail Pengiriman'); ?>
<?php $__env->startSection('page-subtitle', 'Informasi lengkap pengiriman barang'); ?>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600 mt-1"><?php echo e($delivery->delivery_code); ?></p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                ?>
                <span class="badge <?php echo e($statusBadge[$delivery->status] ?? 'bg-gray-100 text-gray-800'); ?> text-sm px-3 py-1.5">
                    <i class="fas <?php echo e($statusIcon[$delivery->status] ?? 'fa-question-circle'); ?> mr-2"></i>
                    <?php echo e($statusText[$delivery->status] ?? ucfirst(str_replace('_', ' ', $delivery->status))); ?>
            ...
            <div class="flex gap-3">
                ...
                <form action="<?php echo e(route('delivery.pickup', $delivery)); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                ...
                <form action="<?php echo e(route('delivery.start', $delivery)); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                ...
                <?php if(in_array($delivery->status, ['on_delivery', 'picked_up', 'assigned'])): ?>
                ...
                <?php if(in_array($delivery->status, ['pending', 'processing', 'assigned'])): ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="p-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-semibold text-gray-900"><?php echo e($delivery->transaction->invoice_number ?? '-'); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Tanggal Transaksi</p>
                            <p class="font-semibold text-gray-900"><?php echo e($delivery->transaction->created_at ? \Carbon\Carbon::parse($delivery->transaction->created_at)->format('d/m/Y H:i') : '-'); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Customer</p>
                            <p class="font-semibold text-gray-900"><?php echo e($delivery->transaction->customer_name ?? 'Pelanggan Umum'); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Total Belanja</p>
                            <p class="font-semibold text-green-600">Rp <?php echo e(number_format($delivery->transaction->total_amount ?? 0, 0, ',', '.')); ?></p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-start mb-6 relative">
                            ...
                            <div class="ml-4 flex-1">
                                ...
                                <p class="text-sm text-gray-600"><?php echo e($delivery->created_at ? \Carbon\Carbon::parse($delivery->created_at)->format('d/m/Y H:i') : '-'); ?></p>
                            </div>
                        </div>

                        <!-- Diproses Logistik -->
                        <div class="flex items-start mb-6 relative">
                            <div class="w-8 h-8 rounded-full <?php echo e(!in_array($delivery->status, ['pending']) ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                <i class="fas <?php echo e(!in_array($delivery->status, ['pending']) ? 'fa-check' : 'fa-clock'); ?> text-sm"></i>
                            </div>
                            <div class="ml-4 flex-1">
                                <h4 class="font-semibold text-gray-900">Diproses Logistik</h4>
                                <?php if(!in_array($delivery->status, ['pending'])): ?>
                                    <p class="text-sm text-gray-600"><?php echo e($delivery->updated_at ? \Carbon\Carbon::parse($delivery->updated_at)->format('d/m/Y H:i') : '-'); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <!-- Ditugaskan ke Kurir -->
                        <div class="flex items-start mb-6 relative">
                            <div class="w-8 h-8 rounded-full <?php echo e($delivery->user_id ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                <i class="fas <?php echo e($delivery->user_id ? 'fa-check' : 'fa-user'); ?> text-sm"></i>
                            </div>
                            <div class="ml-4 flex-1">
                                <h4 class="font-semibold text-gray-900">Ditugaskan ke Kurir</h4>
                                <?php if($delivery->user): ?>
                                    <p class="text-sm font-medium text-gray-900"><?php echo e($delivery->user->name); ?></p>
                                    <p class="text-xs text-gray-500"><?php echo e($delivery->user->phone ?? '-'); ?></p>
                                    <?php if($delivery->assigned_at): ?>
                                        <p class="text-xs text-gray-500 mt-1"><?php echo e(\Carbon\Carbon::parse($delivery->assigned_at)->format('d/m/Y H:i')); ?></p>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <!-- Paket Diambil -->
                        <div class="flex items-start mb-6 relative">
                            <div class="w-8 h-8 rounded-full <?php echo e(in_array($delivery->status, ['picked_up', 'on_delivery', 'delivered']) ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                <i class="fas <?php echo e(in_array($delivery->status, ['picked_up', 'on_delivery', 'delivered']) ? 'fa-check' : 'fa-box'); ?> text-sm"></i>
                            </div>
                            <div class="ml-4 flex-1">
                                <h4 class="font-semibold text-gray-900">Paket Diambil Kurir</h4>
                                <?php if($delivery->pickup_time): ?>
                                    <p class="text-sm text-gray-600"><?php echo e(\Carbon\Carbon::parse($delivery->pickup_time)->format('d/m/Y H:i')); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <!-- Dalam Perjalanan -->
                        <div class="flex items-start mb-6 relative">
                            <div class="w-8 h-8 rounded-full <?php echo e(in_array($delivery->status, ['on_delivery', 'delivered']) ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                <i class="fas <?php echo e(in_array($delivery->status, ['on_delivery', 'delivered']) ? 'fa-check' : 'fa-truck'); ?> text-sm"></i>
                            </div>
                            <div class="ml-4 flex-1">
                                <h4 class="font-semibold text-gray-900">Dalam Perjalanan</h4>
                                <?php if($delivery->start_delivery_time): ?>
                                    <p class="text-sm text-gray-600"><?php echo e(\Carbon\Carbon::parse($delivery->start_delivery_time)->format('d/m/Y H:i')); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start relative">
                            <div class="w-8 h-8 rounded-full <?php echo e($delivery->status === 'delivered' ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                <i class="fas <?php echo e($delivery->status === 'delivered' ? 'fa-check' : 'fa-flag'); ?> text-sm"></i>
                            </div>
                            <div class="ml-4 flex-1">
                                <h4 class="font-semibold text-gray-900">Terkirim</h4>
                                <?php if($delivery->delivered_at): ?>
                                    <p class="text-sm text-gray-600"><?php echo e(\Carbon\Carbon::parse($delivery->delivered_at)->format('d/m/Y H:i')); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="space-y-6">
                ...
                <div class="p-5">
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="font-medium text-gray-900 bg-gray-50 p-2 rounded"><?php echo e($delivery->origin ?? 'Toko Roni Juntinyuat'); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 mb-1">Tujuan</p>
                            <p class="font-medium text-gray-900 bg-gray-50 p-2 rounded"><?php echo e($delivery->destination); ?></p>
                        </div>
                        <?php if($delivery->estimated_delivery_time): ?>
                        <div>
                            <p class="text-sm text-gray-600 mb-1">Estimasi Tiba</p>
                            <p class="font-medium text-gray-900"><?php echo e(\Carbon\Carbon::parse($delivery->estimated_delivery_time)->format('d/m/Y H:i')); ?></p>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="p-5">
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold text-gray-900"><?php echo e($delivery->total_items); ?> item</span>
                        </div>
                        <?php if($delivery->total_weight > 0): ?>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Total Berat</span>
                            <span class="font-medium text-gray-900"><?php echo e(number_format($delivery->total_weight, 2)); ?> kg</span>
                        </div>
                        <?php endif; ?>
                        <?php if($delivery->total_volume > 0): ?>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Total Volume</span>
                            <span class="font-medium text-gray-900"><?php echo e(number_format($delivery->total_volume, 2)); ?> m³</span>
                        </div>
                        <?php endif; ?>
                        <?php if($delivery->delivery_fee > 0): ?>
                        <div class="flex justify-between pt-2 border-t">
                            <span class="text-gray-600">Biaya Kirim</span>
                            <span class="font-semibold text-green-600">Rp <?php echo e(number_format($delivery->delivery_fee, 0, ',', '.')); ?></span>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-5">
                    <p class="text-gray-700 whitespace-pre-line"><?php echo e($delivery->notes); ?></p>
                </div>
            </div>
            <?php endif; ?>
        </div>
...
<div id="assignModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl max-w-md w-full p-6">
        ...
        <form action="<?php echo e(route('delivery.assign', $delivery)); ?>" method="POST">
            <?php echo csrf_field(); ?>
                ...
                <div>
                    ...
                    <select name="vehicle_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        ...
                        <?php $__currentLoopData = $availableVehicles ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vehicle): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($vehicle->id); ?>"><?php echo e($vehicle->name); ?> - <?php echo e($vehicle->license_plate ?? $vehicle->plate_number); ?> (<?php echo e($vehicle->type); ?>)</option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
...
<div id="completeModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl max-w-md w-full p-6">
        ...
        <form action="<?php echo e(route('delivery.complete', $delivery)); ?>" method="POST">
            <?php echo csrf_field(); ?>
...
<div id="cancelModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl max-w-md w-full p-6">
        ...
        <form action="<?php echo e(route('delivery.cancel', $delivery)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <?php echo method_field('POST'); ?>
            <div class="space-y-4">
                <div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel project 3\Toko-Roni-Mobile-App\resources\views/delivery/show.blade.php ENDPATH**/ ?>