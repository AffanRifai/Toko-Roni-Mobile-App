...
<?php $__env->startSection('content'); ?>
        ...
        <div class="container mx-auto px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                        ...
                        <div class="bg-white rounded-xl shadow-sm p-5">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <?php
                                        $paymentColors = [
                                            'cash' => ['bg' => 'bg-green-100', 'text' => 'text-green-800', 'border' => 'border-green-200'],
                                            'debit_card' => ['bg' => 'bg-blue-100', 'text' => 'text-blue-800', 'border' => 'border-blue-200'],
                                            'credit_card' => ['bg' => 'bg-cyan-100', 'text' => 'text-cyan-800', 'border' => 'border-cyan-200'],
                                            'e_wallet' => ['bg' => 'bg-purple-100', 'text' => 'text-purple-800', 'border' => 'border-purple-200'],
                                        ];
                                        $paymentIcons = [
                                        ...
                                        $paymentConfig = $paymentColors[$transaction->payment_method] ?? ['bg' => 'bg-gray-100', 'text' => 'text-gray-800', 'border' => 'border-gray-200'];
                                        $paymentIcon = $paymentIcons[$transaction->payment_method] ?? 'credit-card';
                                    ?>
                                    <span class="px-3 py-1 rounded-full text-sm font-medium <?php echo e($paymentConfig['bg']); ?> <?php echo e($paymentConfig['text']); ?> border <?php echo e($paymentConfig['border']); ?>">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700 uppercase tracking-wider">#</th>
                                        <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700 uppercase tracking-wider">Produk</th>
                                        <th class="text-center py-3 px-4 text-sm font-semibold text-gray-700 uppercase tracking-wider">Harga</th>
                                        <th class="text-center py-3 px-4 text-sm font-semibold text-gray-700 uppercase tracking-wider">Qty</th>
                                        <th class="text-right py-3 px-4 text-sm font-semibold text-gray-700 uppercase tracking-wider">Subtotal</th>
                                        <?php if($transaction->need_delivery): ?>
                                        <th class="text-center py-3 px-4 text-sm font-semibold text-gray-700 uppercase tracking-wider">Status</th>
                                        <?php endif; ?>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200">
                                    <?php
                                        // Ambil items_to_deliver dari delivery jika ada
                                        $itemsToDeliverIds = [];
                                        if($transaction->delivery && $transaction->delivery->items_to_deliver) {
                                            $itemsToDeliver = is_array($transaction->delivery->items_to_deliver) 
                                                ? $transaction->delivery->items_to_deliver 
                                                : json_decode($transaction->delivery->items_to_deliver, true);
                                            $itemsToDeliverIds = collect($itemsToDeliver)->pluck('id')->toArray();
                                        }
                                    ?>
                                    <?php $__currentLoopData = $transaction->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php
                                            $isDelivered = in_array($item->product_id, $itemsToDeliverIds);
                                        ?>
                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                            ...
                                            <td class="py-4 px-4">
                                                <div class="flex items-center">
                                                    ...
                                                    <div>
                                                        <div class="font-semibold text-gray-900"><?php echo e($item->product->name ?? 'Produk tidak ditemukan'); ?></div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4 space-y-2">
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-medium text-gray-900">
                                                <?php echo e($transaction->delivery->recipient_name ?? $transaction->customer_name); ?>
                                        ...
                                        <div class="flex justify-between">
                                            <span class="text-gray-600">Telepon:</span>
                                            <span class="font-medium text-gray-900">
                                                <?php echo e($transaction->delivery->recipient_phone ?? ($transaction->customer_phone ?? '-')); ?>
                                        ...
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-medium text-gray-900 text-right">
                                                <?php echo e($transaction->delivery->destination ?? $transaction->delivery_address ?? '-'); ?>
                                        ...
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-medium text-gray-900">
                                                <?php echo e(\Carbon\Carbon::parse($transaction->desired_delivery_date)->format('d/m/Y')); ?>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        <?php
                                            $totalSemuaBarang = $transaction->items->sum('qty');
                                            $totalDikirim = 0;
                                            $totalDibawa = 0;
                                            $itemsToDeliver = [];
                                            
                                            if($transaction->delivery) {
                                                // Jika status sudah delivered, anggap SEMUA barang terkirim
                                                if($transaction->delivery->status == 'delivered') {
                                                    $totalDikirim = $totalSemuaBarang;
                                                    // Buat itemsToDeliver dari semua item transaksi
                                                ...
                                                // Jika belum delivered, ambil dari database
                                                elseif($transaction->delivery->items_to_deliver) {
                                                    $itemsToDeliver = is_array($transaction->delivery->items_to_deliver) 
                                                        ? $transaction->delivery->items_to_deliver 
                                                        : json_decode($transaction->delivery->items_to_deliver, true);
                                                    $totalDikirim = collect($itemsToDeliver)->sum('qty');
                                                }
                                            }
                                            
                                            $totalDibawa = $totalSemuaBarang - $totalDikirim;
                                        ?>
                                        
                                        <!-- Barang Dikirim -->
                                        <div class="mb-4">
                                            <div class="flex justify-between items-center pb-2 border-b border-gray-200">
                                                <span class="text-gray-600 font-medium">🚚 Barang Dikirim:</span>
                                                <span class="font-semibold text-blue-600"><?php echo e($totalDikirim); ?> item</span>
                                            </div>
                                            <?php if(!empty($itemsToDeliver) && count($itemsToDeliver) > 0): ?>
                                                <div class="mt-2 space-y-1">
                                                    <?php $__currentLoopData = $itemsToDeliver; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        <div class="flex justify-between text-sm">
                                                            <span class="text-gray-700"><?php echo e($item['name'] ?? 'Produk'); ?></span>
                                                            <span class="font-medium text-gray-900"><?php echo e($item['qty'] ?? 0); ?> pcs</span>
                                                        </div>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                </div>
                                            <?php else: ?>
                                                <p class="text-sm text-gray-500 mt-2">Tidak ada barang yang dikirim</p>
                                            <?php endif; ?>
                                        </div>
                                        ...
                                        <div class="mt-3 pt-2 border-t border-gray-200">
                                            <div class="flex justify-between items-center">
                                                <span class="text-gray-600 font-medium">🛍️ Barang Dibawa:</span>
                                                <span class="font-semibold text-green-600"><?php echo e($totalDibawa); ?> item</span>
                                            </div>
                                            <?php if($totalDibawa > 0): ?>
                                                <div class="mt-2 space-y-1">
                                                    <?php $__currentLoopData = $transaction->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        <?php
                                                            $isDelivered = false;
                                                            if(!empty($itemsToDeliver)) {
                                                                $isDelivered = collect($itemsToDeliver)->contains('id', $item->product_id);
                                                            }
                                                        ?>
                                                        <?php if(!$isDelivered): ?>
                                                            <div class="flex justify-between text-sm">
                                                                <span class="text-gray-700"><?php echo e($item->product->name ?? 'Produk'); ?></span>
                                                                <span class="font-medium text-gray-900"><?php echo e($item->qty); ?> pcs</span>
                                                            </div>
                                                        <?php endif; ?>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                </div>
                                            <?php else: ?>
                                                <p class="text-sm text-gray-500 mt-2">Semua barang dikirim</p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-4 space-y-3">
                                        ...
                                        <div class="flex justify-between items-center bg-blue-50 rounded-lg p-3">
                                            ...
                                            <span class="font-bold text-blue-600 text-lg">Rp <?php echo e(number_format($transaction->delivery_fee, 0, ',', '.')); ?></span>
                                        </div>
                                        <?php endif; ?>
                        ...
                        <div class="p-5">
                            <div class="max-w-2xl mx-auto">
                                <div class="bg-gradient-to-br from-white to-gray-50 rounded-lg border border-gray-200 p-6">
                                    ...
                                    <?php if($transaction->discount > 0): ?>
                                        <div class="flex justify-between items-center py-2 text-red-600">
                                            ...
                                            <span class="font-medium">
                                                - Rp <?php echo e(number_format(($transaction->items->sum('subtotal') * $transaction->discount) / 100, 0, ',', '.')); ?>
                                    ...
                                    <!-- Total -->
                                    <div class="flex justify-between items-center py-4 border-t border-b border-gray-200 my-2">
                                        <span class="font-bold text-lg text-gray-900">Total Bayar</span>
                                        <span class="font-bold text-2xl text-blue-600">
                                            Rp <?php echo e(number_format($transaction->total_amount + ($transaction->delivery_fee ?? 0), 0, ',', '.')); ?>
                ...
                <div class="lg:w-1/3 space-y-6">
                   ...
                   <!-- ========== FITUR PENGIRIMAN ========== -->
                    <?php if(auth()->user()->role === 'kasir' || auth()->user()->role === 'owner' || auth()->user()->role === 'manager'): ?>
                        <?php if($transaction->need_delivery && !$transaction->delivery): ?>
                            ...
                            <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-green-500">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="bg-blue-50 rounded-lg p-3 text-sm">
                                        ...
                                        Status: <span class="font-semibold text-blue-700">Menunggu Konfirmasi Logistik</span>
                                    </div>
                        ...
                        <?php elseif(!$transaction->need_delivery && !$transaction->delivery): ?>
                            <!-- Form Pengiriman (Hanya untuk Kasir/Owner/Manager) -->
                            <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-blue-500">
                                ...
                                <form id="deliveryForm" action="<?php echo e(route('delivery.request', $transaction)); ?>" method="POST" class="space-y-4">
                                    <?php echo csrf_field(); ?>
                                        ...
                                        <div class="space-y-3">
                                            <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                                                <input type="radio" name="need_delivery" value="yes" class="mr-3" 
                                                    <?php echo e(old('need_delivery') == 'yes' ? 'checked' : ''); ?>

                                                    onchange="toggleDeliveryOptions(true)">
                                                <div>
                                                    <span class="font-medium text-gray-900">Ya, perlu dikirim</span>
                                                    <p class="text-xs text-gray-500">Pesanan akan diantar ke alamat pelanggan</p>
                                                </div>
                                            </label>

                                            <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                                                <input type="radio" name="need_delivery" value="no" class="mr-3" 
                                                    <?php echo e(old('need_delivery', 'no') == 'no' ? 'checked' : ''); ?>

                                                    onchange="toggleDeliveryOptions(false)">
                                                <div>
                                    ...
                                    <!-- Delivery Options -->
                                    <div id="deliveryOptions" class="space-y-4 <?php echo e(old('need_delivery') == 'yes' ? '' : 'hidden'); ?>">
                                        <!-- Data Penerima -->
                                        <div class="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <input type="text" name="recipient_name" id="recipient_name"
                                                    value="<?php echo e(old('recipient_name', $transaction->customer_name)); ?>"
                                            ...
                                            <div>
                                                ...
                                                <input type="text" name="recipient_phone" id="recipient_phone"
                                                    value="<?php echo e(old('recipient_phone', $transaction->customer_phone)); ?>"
                                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="delivery_address" id="delivery_address" rows="2"
                                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                                placeholder="Masukkan alamat lengkap"><?php echo e(old('delivery_address', $transaction->customer_address ?? '')); ?></textarea>
                                        </div>

                                        <div>
                                            <label for="desired_delivery_date" class="block text-sm font-medium text-gray-700 mb-2">
                                                Tanggal Pengiriman <span class="text-red-500">*</span>
                                            </label>
                                            <input type="date" name="desired_delivery_date" id="desired_delivery_date"
                                                value="<?php echo e(old('desired_delivery_date', now()->addDays(2)->format('Y-m-d'))); ?>"
                                        ...
                                        <div>
                                            ...
                                            <div class="bg-gray-50 rounded-lg p-3 max-h-48 overflow-y-auto">
                                                ...
                                                <label class="flex items-center p-2 hover:bg-white rounded cursor-pointer">
                                                    ...
                                                    <span class="flex-1 text-sm"><?php echo e($item->product->name ?? 'Produk'); ?></span>
                                                    <span class="text-sm font-medium text-gray-700"><?php echo e($item->qty); ?> pcs</span>
                                                </label>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            </div>
                                            <p class="text-xs text-gray-500 mt-1">*Kosongkan jika semua barang akan dikirim</p>
                                        </div>

                                        <!-- Pilih Kendaraan -->
                                        <div>
                                            <label for="vehicle_type" class="block text-sm font-medium text-gray-700 mb-2">
                                                Pilih Jenis Kendaraan <span class="text-red-500">*</span>
                                            </label>
                                            <select name="vehicle_type" id="vehicle_type" 
                                                ...
                                                <option value="motorcycle" <?php echo e(old('vehicle_type') == 'motorcycle' ? 'selected' : ''); ?>>Motor</option>
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="delivery_fee" id="delivery_fee" 
                                                min="0" step="1000" value="<?php echo e(old('delivery_fee', 0)); ?>"
                                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="delivery_notes" id="delivery_notes" rows="2"
                                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                                placeholder="Catatan tambahan untuk kurir"><?php echo e(old('delivery_notes')); ?></textarea>
                                        </div>
                        ...
                        <?php elseif($transaction->delivery): ?>
                            ...
                            <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-green-500">
                                ...
                                <?php if(auth()->user()->role === 'kasir'): ?>
                                    <!-- Tampilan sederhana untuk kasir (read-only) -->
                                    <div class="space-y-4">
                                        <div class="bg-blue-50 rounded-lg p-4">
                                            <div class="flex items-start gap-3">
                                                <i class="fas fa-info-circle text-blue-600 mt-0.5"></i>
                                                <div>
                                                    <p class="text-sm text-blue-800 font-medium">Status Pengiriman</p>
                                                    <p class="text-sm text-blue-700 mt-1">
                                                        Pengiriman sedang diproses oleh tim logistik. 
                                                        Status akan diperbarui secara otomatis.
                                                    </p>
                                                    <?php if($transaction->delivery->status == 'delivered'): ?>
                                                        <p class="text-sm text-green-700 mt-2">
                                                            <i class="fas fa-check-circle mr-1"></i>
                                                            Pesanan telah terkirim pada <?php echo e(\Carbon\Carbon::parse($transaction->delivery->delivered_at)->format('d/m/Y H:i')); ?>
                                        ...
                                        <!-- Info ringkasan untuk kasir -->
                                        <div class="bg-gray-50 rounded-lg p-4 space-y-2">
                                            <div class="flex justify-between text-sm">
                                                <span class="text-gray-600">Kode Pengiriman:</span>
                                                <span class="font-medium text-gray-900"><?php echo e($transaction->delivery->delivery_code ?? '-'); ?></span>
                                            </div>
                                            <div class="flex justify-between text-sm">
                                                <span class="text-gray-600">Alamat Tujuan:</span>
                                                <span class="font-medium text-gray-900 text-right"><?php echo e($transaction->delivery->destination ?? '-'); ?></span>
                                            </div>
                                            <?php if($transaction->delivery_fee > 0): ?>
                                            <div class="flex justify-between text-sm">
                                                <span class="text-gray-600">Biaya Kirim:</span>
                                                <span class="font-medium text-blue-600">Rp <?php echo e(number_format($transaction->delivery_fee, 0, ',', '.')); ?></span>
                                            </div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                <?php else: ?>
                                    <!-- Tampilan lengkap untuk owner/manager/logistik (dengan timeline) -->
                                    <div class="space-y-4">
                                        <!-- Timeline Pengiriman -->
                                        <div class="relative">
                                            ...
                                            <div class="flex items-start mb-6 relative">
                                                ...
                                                <div class="ml-4 flex-1">
                                                    ...
                                                    <p class="text-xs text-gray-500"><?php echo e($transaction->delivery->created_at ? $transaction->delivery->created_at->format('d/m/Y H:i') : '-'); ?></p>
                                                </div>
                                            </div>

                                            <!-- Diproses Logistik -->
                                            <div class="flex items-start mb-6 relative">
                                                <div class="w-8 h-8 rounded-full <?php echo e(!in_array($transaction->delivery->status, ['pending']) ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                                    <i class="fas <?php echo e(!in_array($transaction->delivery->status, ['pending']) ? 'fa-check' : 'fa-clock'); ?> text-sm"></i>
                                                </div>
                                                <div class="ml-4 flex-1">
                                                    ...
                                                    <?php if($transaction->delivery->updated_at && $transaction->delivery->status != 'pending'): ?>
                                                        <p class="text-xs text-gray-500"><?php echo e($transaction->delivery->updated_at->format('d/m/Y H:i')); ?></p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                            ...
                                            <!-- Ditugaskan ke Kurir -->
                                            <div class="flex items-start mb-6 relative">
                                                <div class="w-8 h-8 rounded-full <?php echo e($transaction->delivery->user_id ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                                    <i class="fas <?php echo e($transaction->delivery->user_id ? 'fa-check' : 'fa-user'); ?> text-sm"></i>
                                                </div>
                                                <div class="ml-4 flex-1">
                                                    <h4 class="font-semibold text-gray-900">Ditugaskan ke Kurir</h4>
                                                    <?php if($transaction->delivery->user): ?>
                                                        <p class="text-sm text-gray-800"><?php echo e($transaction->delivery->user->name ?? 'N/A'); ?></p>
                                                        <p class="text-xs text-gray-500"><?php echo e($transaction->delivery->user->phone ?? '-'); ?></p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-start mb-6 relative">
                                                <div class="w-8 h-8 rounded-full <?php echo e($transaction->delivery->status == 'delivered' ? 'bg-green-500' : 'bg-orange-500'); ?> flex items-center justify-center text-white z-10">
                                                    <i class="fas fa-truck text-sm"></i>
                                                </div>
                                                <div class="ml-4 flex-1">
                                                    <h4 class="font-semibold text-gray-900">Dalam Perjalanan</h4>
                                                    <?php if($transaction->delivery->start_delivery_time): ?>
                                                        <p class="text-xs text-gray-500"><?php echo e(\Carbon\Carbon::parse($transaction->delivery->start_delivery_time)->format('d/m/Y H:i')); ?></p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                            <?php endif; ?>
                                            ...
                                            <div class="flex items-start relative">
                                                <div class="w-8 h-8 rounded-full <?php echo e($transaction->delivery->status == 'delivered' ? 'bg-green-500' : 'bg-gray-300'); ?> flex items-center justify-center text-white z-10">
                                                    <i class="fas <?php echo e($transaction->delivery->status == 'delivered' ? 'fa-check' : 'fa-truck'); ?> text-sm"></i>
                                                </div>
                                                <div class="ml-4 flex-1">
                                                    <h4 class="font-semibold text-gray-900">Terkirim</h4>
                                                    <?php if($transaction->delivery->delivered_at): ?>
                                                        <p class="text-xs text-gray-500"><?php echo e(\Carbon\Carbon::parse($transaction->delivery->delivered_at)->format('d/m/Y H:i')); ?></p>
                                                    <?php endif; ?>
                                                    <?php if($transaction->delivery->recipient_name): ?>
                                                        <p class="text-xs text-gray-600 mt-1">Penerima: <?php echo e($transaction->delivery->recipient_name); ?></p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                                            <div class="flex justify-between text-sm">
                                                ...
                                                <span class="font-medium text-gray-900"><?php echo e(ucfirst($transaction->delivery->vehicle_type ?? 'motor')); ?></span>
                                            </div>
                                            <div class="flex justify-between text-sm">
                                                <span class="text-gray-600">Alamat:</span>
                                                <span class="font-medium text-gray-900 text-right"><?php echo e($transaction->delivery->address ?? ($transaction->delivery->destination ?? '-')); ?></span>
                                            </div>
                                            <?php if($transaction->delivery->notes): ?>
                                            <div class="text-sm">
                                                <span class="text-gray-600">Catatan:</span>
                                                <p class="text-gray-800 mt-1 bg-white p-2 rounded"><?php echo e($transaction->delivery->notes); ?></p>
                                            </div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <!-- ID Transaksi -->
                            <div class="bg-gradient-to-br from-white to-blue-50 border border-blue-100 rounded-lg p-4 text-center transition-transform duration-300 hover:-translate-y-1">
                            ...
                            <!-- Total Item -->
                            <div class="bg-gradient-to-br from-white to-green-50 border border-green-100 rounded-lg p-4 text-center transition-transform duration-300 hover:-translate-y-1">
                            ...
                            <!-- Jenis Produk -->
                            <div class="bg-gradient-to-br from-white to-yellow-50 border border-yellow-100 rounded-lg p-4 text-center transition-transform duration-300 hover:-translate-y-1">
                            ...
                            <!-- Waktu -->
                            <div class="bg-gradient-to-br from-white to-cyan-50 border border-cyan-100 rounded-lg p-4 text-center transition-transform duration-300 hover:-translate-y-1">
                                <div class="text-cyan-500 mb-2">
                                    <i class="fas fa-clock text-2xl"></i>
                                </div>
                                <div class="font-bold text-lg text-gray-900"><?php echo e($transaction->created_at->diffForHumans()); ?></div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5">
                        ...
                        <div class="bg-gradient-to-br from-white to-gray-50 rounded-lg border border-gray-200 p-4">
                            <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                ...
                                <span class="font-medium text-gray-900"><?php echo e($transaction->created_at->format('d/m/Y')); ?></span>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                <span class="text-gray-600">Jam</span>
                                <span class="font-medium text-gray-900"><?php echo e($transaction->created_at->format('H:i')); ?></span>
                            </div>
                            <div class="flex justify-between items-center py-2">
                                <span class="text-gray-600">Lokasi</span>
                                <span class="font-medium text-gray-900">Kasir <?php echo e($transaction->user->name ?? 'N/A'); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <form id="createDeliveryForm" action="<?php echo e(route('delivery.request', $transaction)); ?>" method="POST" class="hidden">
        ...
        <input type="hidden" name="recipient_name" value="<?php echo e($transaction->recipient_name ?? $transaction->customer_name); ?>">
        <input type="hidden" name="recipient_phone" value="<?php echo e($transaction->recipient_phone ?? $transaction->customer_phone); ?>">
    ...
    <script>
        ...
        (function() {
            ...
            /**
             * Toggle delivery options visibility and change button behavior
            ...
            window.toggleDeliveryOptions = function(show) {
                ...
                const submitButton = document.querySelector('#deliveryForm button[type="submit"]');
                
                if (options) {
                    if (show) {
                        options.style.display = 'block';
                        // Ubah tombol submit kembali ke "Kirim ke Logistik"
                        if (submitButton) {
                            submitButton.innerHTML = '<i class="fas fa-paper-plane mr-2"></i> Kirim ke Logistik';
                            submitButton.style.display = 'block';
                        }
                        
                        // Tambahkan required attributes
                        const requiredFields = ['recipient_name', 'delivery_address', 'desired_delivery_date', 'vehicle_type'];
                        ...
                        // Hapus hidden input untuk need_delivery=no
                        const hiddenNoInput = document.querySelector('input[name="need_delivery_no_hidden"]');
                        if (hiddenNoInput) hiddenNoInput.remove();
                        
                    } else {
                        options.style.display = 'none';
                        // Ubah tombol submit menjadi "Kembali ke Transaksi"
                        if (submitButton) {
                            submitButton.innerHTML = '<i class="fas fa-arrow-left mr-2"></i> Kembali ke Transaksi';
                        }
                        ...
                        requiredFields.forEach(fieldName => {
                            ...
                            if (field) {
                                ...
                                // Kosongkan nilai field agar tidak ikut terkirim
                        ...
                        // Kosongkan textarea
                        const textarea = document.querySelector('textarea[name="delivery_address"]');
                        if (textarea) textarea.value = '';
                        
                        // Kosongkan select
                        const select = document.querySelector('select[name="vehicle_type"]');
            ...
            function validateDeliveryForm(form) {
                ...
                // Jika memilih "Tidak perlu dikirim", submit langsung tanpa validasi
                if (needDelivery.value === 'no') {
                    return true;
                }
            ...
            /**
             * Handle form submission based on selection
             */
            function handleFormSubmission(form) {
                ...
                // Jika memilih "Tidak perlu dikirim", redirect ke halaman transaksi
                if (needDelivery.value === 'no') {
                    window.location.href = "<?php echo e(route('transactions.create', $transaction)); ?>";
                    return false;
                }
            ...
            function initDeliveryOptions() {
                ...
                if (needDeliveryYes && needDeliveryYes.checked) {
                    toggleDeliveryOptions(true);
                } else if (needDeliveryNo && needDeliveryNo.checked) {
            ...
            function initFormSubmission() {
                ...
                form.addEventListener('submit', function(e) {
                    // Handle submission based on selection
                    ...
                    if (submitBtn) {
                        ...
                        // Re-enable after 30 seconds in case of slow network
                        setTimeout(() => {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel project 3\Toko-Roni-Mobile-App\resources\views/transactions/show.blade.php ENDPATH**/ ?>